    ...
    const fetchWriterVideos = async () => {
        try {
            ...
            const completed = data.filter(v =>
                (v.currentDepartment === 'writer' && (v.status === 'department_completed' || v.status === 'waiting_approval')) ||
                (isMainTeam && v.previousDepartment === 'writer' && v.forwardedAt && (now - new Date(v.forwardedAt)) < 3600000)
            );
    ...
    return (
        <div className={styles.pageContainer}>
                ...
                <div className={styles.gridContainer}>
                    <div className={styles.taskListColumn}>
                        ...
                        <ProjectList
                            ...
                            onForwardClick={(id) => {
                                const video = completedWriterVideos.find(v => v.id === id);
            ...
            <ConfirmActionModal
                ...
                title="Forward Task"
                description={`Are you sure you want to forward "${videoToForward?.name}" to the Speaker department?`}
